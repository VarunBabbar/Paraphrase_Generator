...
import sys
sys.path.append("C:\\Users\\Puria\\puria-radmard\\PQG-pytorch")
...
def main():
    ...
    pgen.load_state_dict(torch.load("save/model_epoch_0"))                              # REMEMBER TO CHANGE

    # setup logging
    logger = SummaryWriter(os.path.join(LOG_DIR, TIME + args.name))
    subprocess.run(['mkdir', os.path.join(GEN_DIR, TIME)], check=False, shell=True)
    ...
    for epoch_ in range(op["epochs"]):
        ...
        save_model(pgen, pgen_optim, epoch, "save/model_epoch_{}".format(epoch))
